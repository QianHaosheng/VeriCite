...
def make_doc_prompt(doc, doc_id, doc_prompt, short):
    if short == "snippet":
        text = doc["extraction"]
    elif short == "summary":
        text = doc["summary"]
...
def make_demo(item, prompt, ndoc=None, doc_prompt=None, instruction=None, test=False, short=None):
    message = []
    ...
    if "{D}" in prompt:
        if ndoc == 0:
            ...
            text = "".join([make_doc_prompt(doc, doc_id, doc_prompt, short) for doc_id, doc in enumerate(doc_list)])
